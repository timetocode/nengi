...
type CulledEcsChannelUnderTest = {
    createEntity(): number
...
type Scenario = {
    context: Context
    instance: Instance
    channel: CulledEcsChannelUnderTest
    Transform: any
    Body: any
    users: User[]
...
type Channel3DUnderTest = {
    createEntity(): number
    addSpatialComponent<T extends { nid: number, ntype: number }>(pid: number, component: T): T & { pid: number }
    addComponent<T extends { nid: number, ntype: number }>(pid: number, component: T): T & { pid: number }
    removeEntity(pid: number): number
    subscribe(user: User, view: AABB3D): void
    updateView(user: User, view: AABB3D): void
    createComponentWriter(ntype: number, schema: any): any
}
...
function createScenario(createChannel: (instance: Instance) => CulledEcsChannelUnderTest): Scenario {
...
function createScenario3D(createChannel: (instance: Instance) => Channel3DUnderTest): Scenario3D {
...
function runScriptedCorrectnessScenario(createChannel: (instance: Instance) => CulledEcsChannelUnderTest) {
...
function runDeterministicFuzzScenario(createChannel: (instance: Instance) => CulledEcsChannelUnderTest) {
    const scenario = createScenario(createChannel)
...
function runQueuedSnapshotScenario(createChannel: (instance: Instance) => CulledEcsChannelUnderTest) {
    const scenario = createScenario(createChannel)
    const a = addRoot(scenario, 5, 5, 100)
...
function run3DCorrectnessScenario(createChannel: (instance: Instance) => Channel3DUnderTest) {
    const scenario = createScenario3D(createChannel)
...
describe('ECS spatial snapshot correctness', () => {
    it('keeps EcsChannel2D clients synchronized with the cell-coarse oracle', () => {
        runScriptedCorrectnessScenario(instance => new EcsChannel2D(instance.localState, 10, { name: 'ecs-channel-correctness' }))
    })

    it('keeps EcsChannel2D synchronized during deterministic spatial churn', () => {
        runDeterministicFuzzScenario(instance => new EcsChannel2D(instance.localState, 10, { name: 'ecs-channel-fuzz' }))
    })

    it('keeps EcsChannel2D synchronized when queued snapshots are drained later', () => {
        runQueuedSnapshotScenario(instance => new EcsChannel2D(instance.localState, 10, { name: 'ecs-channel-queued' }))
    })

    it('keeps EcsChannel3D clients synchronized with the cell-coarse oracle', () => {
        run3DCorrectnessScenario(instance => new EcsChannel3D(instance.localState, 10, { name: 'ecs-channel-3d-correctness' }))
    })
})
