...
function trackBody(history: Historian3D, body: Body, timeMs?: number) {
    const options = {
        nid: 'nid' as const,
        x: 'x' as const,
        y: 'y' as const,
...
describe('Historian3D', () => {
    it('records compact spatial samples without retaining live object references', () => {
        const history = new Historian3D({ retentionMs: 1000 })
    ...
    it('returns the same nearest query results with and without the grid index', () => {
        const brute = new Historian3D({ retentionMs: 1000 })
        const indexed = new Historian3D({
            retentionMs: 1000,
        ...
        const ids = (samples: { nid: number }[]) => samples.map(sample => sample.nid).sort((a, b) => a - b)
        const hitIds = (hits: { sample: { nid: number } }[]) => hits.map(hit => hit.sample.nid).sort((a, b) => a - b)
