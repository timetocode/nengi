...
export type Ecs2DComponent = IEntity & { pid: number }

export type Ecs2DUpdateLog = EcsSpatialManualUpdateLog

type Cell = SpatialGridCell<Ecs2DComponent> & Ecs2DUpdateLog

export type Ecs2DTypeWriters = {
    [name: string]: any
    readonly ntype: number
    readonly schema: Schema
    readonly props: { [name: string]: (component: Ecs2DComponent, value: any) => void }
    readonly groups: { [name: string]: (component: Ecs2DComponent, ...values: any[]) => void }
}

export type EcsChannel2DOptions = {
    name?: string
...
function createUpdateLog(): Ecs2DUpdateLog {
    return createEcsSpatialManualUpdateLog()
}

function initializeEcsSpatialCell(cell: SpatialGridCell<Ecs2DComponent>) {
    Object.assign(cell, createUpdateLog())
}

const EMPTY_NIDS: number[] = []

export type EcsChannelUserSnapshot = {
    toCreate: number[]
    toDelete: number[]
    group: EcsChannelVisibilityGroup
}

export type EcsChannelVisibilityGroup = {
    cellSignature: string
    visibleCellKeys: string[]
    visibleNids: number[]
    visibleNidSet?: Set<number>
    users: User[]
    stableSnapshot?: EcsChannelUserSnapshot
...
export type EcsChannelVisibilityPlan = {
    tick: number
    groups: EcsChannelVisibilityGroup[]
    userSnapshots: Map<number, EcsChannelUserSnapshot>
}
...
/**
 * Culled 2D ECS channel with manual mutation emission. Userland must call the
 * component writers for networked updates; nengi does not autodiff component state.
 */
export class EcsChannel2D {
    ...
    headerVersion = 0
    channelType = ChannelType.EcsChannel2D
    ...
    private entities: EcsSpatialEntityStore<Ecs2DComponent>
    private views: Map<number, SpatialView> = new Map()
    private viewVersions: Map<number, number> = new Map()
    private grid: SpatialGrid2D<Ecs2DComponent>
    ...
    private pendingPropComponents: Ecs2DComponent[] = []
    private pendingPropSchemas: SchemaProp[] = []
    private pendingPropValues: any[] = []
    private pendingGroupComponents: Ecs2DComponent[] = []
    ...
    constructor(localState: LocalState, cellSize: number, options: EcsChannel2DOptions = {}) {
        if (!Number.isFinite(cellSize) || cellSize <= 0) {
            throw new Error('EcsChannel2D requires a positive finite cell size.')
        }
        if (options.queryPadding !== undefined && (!Number.isFinite(options.queryPadding) || options.queryPadding < 0)) {
            throw new Error('EcsChannel2D queryPadding must be a non-negative finite number.')
        }
    ...
    private addRootToCell(pid: number, component: Ecs2DComponent) {
    ...
    private validatePendingWriteComponent(component: Ecs2DComponent) {
        if (this.grid.objectCells.has(component.pid)) {
            return true
        }
        if (this.validateManualWriteTargets) {
            throw new Error(`EcsChannel2D cannot write mutation for component nid ${component.nid}; no spatial cell was found for pid ${component.pid}.`)
        }
        return false
    }

    private getCellForPendingWrite(component: Ecs2DComponent, updatedPids: Set<number>) {
    ...
    private appendPendingProp(component: Ecs2DComponent, prop: SchemaProp, value: any) {
    ...
    private appendPendingGroup(ntype: number, component: Ecs2DComponent, group: SchemaUpdateGroup, values: IArguments | any[], valueOffset = 0) {
        if (!this.validatePendingWriteComponent(component)) {
            return
        }
    ...
    private writeManualPropMutation(cell: Ecs2DUpdateLog, component: Ecs2DComponent, prop: SchemaProp, value: any) {
    ...
    addComponent<T extends IEntity>(pid: number, component: T, options: { spatial?: boolean } = {}): T & Ecs2DComponent {
    ...
    private removeComponentInternal(componentOrNid: Ecs2DComponent | number) {
    ...
    updateSpatialComponent(componentOrNid: Ecs2DComponent | number) {
    ...
    createComponentWriter(ntype: number, schema: Schema): Ecs2DTypeWriters {
        const props: Ecs2DTypeWriters['props'] = Object.create(null)
        const groups: Ecs2DTypeWriters['groups'] = Object.create(null)
        const writers: Ecs2DTypeWriters = { ntype, schema, props, groups }
        ...
        for (let i = 0; i < propNames.length; i++) {
            ...
            props[name] = (component: Ecs2DComponent, value: any) => {
                this.appendPendingProp(component, prop, value)
            }
            addAlias(name, props[name])
        }

        const writeGroup = (component: Ecs2DComponent, group: SchemaUpdateGroup, values: IArguments | any[]) => {
        ...
        for (let i = 0; i < schema.updateGroups.length; i++) {
            ...
            groups[group.name] = function writeEcsSpatialGroup(component: Ecs2DComponent) {
