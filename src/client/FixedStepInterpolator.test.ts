import { FixedStepInterpolator, InterpolationStatus } from './FixedStepInterpolator'
import {
    applyTestSnapshot,
    createInterpolationTestClient,
    createTestSnapshot,
    InterpolationTestHarness
} from './InterpolationTestHarness'
...
describe('FixedStepInterpolator', () => {
    ...
    it('does not move the sampled target tick backward when clock offset samples jitter', () => {
        ...
        expect(second.status).toBe(InterpolationStatus.Ok)
    ...
    it('returns explicit statuses instead of extrapolating', () => {
        ...
        expect(harness.sample(100, 1075).status).toBe(InterpolationStatus.BeforeHistory)
        const held = harness.sample(100, 1250)
        expect(held.status).toBe(InterpolationStatus.Ok)
    ...
    it('changing interpolation delay changes the sampled target tick immediately', () => {
        ...
        expect(slower.status).toBe(InterpolationStatus.Ok)
        expect(faster.status).toBe(InterpolationStatus.Ok)
    ...
    it('does not treat sustained latency offset as adaptive delay', () => {
        const lowLatency = new InterpolationTestHarness({ delay: { mode: 'adaptive', minMs: 10, safetyTicks: 0 } })
        const highLatency = new InterpolationTestHarness({ delay: { mode: 'adaptive', minMs: 10, safetyTicks: 0 } })
        ...
        expect(lowLatencySample.status).toBe(InterpolationStatus.Ok)
        expect(highLatencySample.status).toBe(InterpolationStatus.Ok)
    ...
    it('keeps the target buffer stable when frames arrive faster than their fixed server cadence', () => {
        ...
        expect(latestSample.status).toBe(InterpolationStatus.Ok)
