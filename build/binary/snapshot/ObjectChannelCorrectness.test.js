...
function visible2D(entity, view) {
...
function visible3D(entity, view) {
    const cellX = cellCoord(entity.x, 10);
    const cellY = cellCoord(entity.y, 10);
...
function assertClientMatchesOracle(scenario, clientIndex) {
    var _a, _b, _c, _d;
    ...
    for (let i = 0; i < scenario.records.length; i++) {
        ...
        expect((_a = client.store.get(record.entity.nid)) === null || _a === void 0 ? void 0 : _a.x).toBe(record.entity.x);
        expect((_b = client.store.get(record.entity.nid)) === null || _b === void 0 ? void 0 : _b.y).toBe(record.entity.y);
        expect((_c = client.store.get(record.entity.nid)) === null || _c === void 0 ? void 0 : _c.z).toBe(record.entity.z);
        expect((_d = client.store.get(record.entity.nid)) === null || _d === void 0 ? void 0 : _d.hp).toBe(record.entity.hp);
    }
...
function addEntity(scenario, x, y, z, hp) {
...
function removeEntity(scenario, record) {
...
function runObjectScenario(scenario) {
    var _a, _b, _c, _d;
    ...
    (_b = (_a = scenario.channel).updateView) === null || _b === void 0 ? void 0 : _b.call(_a, scenario.users[0], scenario.views[0]);
    stepAndAssert(scenario);
    ...
    (_d = (_c = scenario.channel).updateView) === null || _d === void 0 ? void 0 : _d.call(_c, scenario.users[1], scenario.views[1]);
    stepAndAssert(scenario);
...
function runDeterministicChurnScenario(scenario, options) {
    var _a, _b, _c, _d;
    const random = createRandom(0xfeed1234);
    for (let i = 0; i < 24; i++) {
        addEntity(scenario, Math.floor(random() * 260) - 40, Math.floor(random() * 180) - 40, options.is3D ? Math.floor(random() * 160) - 40 : 5, 1000 + i);
    }
    stepAndAssert(scenario);
    for (let tick = 1; tick <= 80; tick++) {
        ...
        for (let i = 0; i < scenario.records.length; i++) {
            ...
            if (random() < 0.35) {
                scenario.move(record.entity, Math.floor(random() * 300) - 50, Math.floor(random() * 220) - 50, options.is3D ? Math.floor(random() * 220) - 50 : record.entity.z);
            }
            if (random() < 0.45) {
...
function runQueuedSnapshotScenario(scenario, options) {
    var _a, _b;
    ...
    scenario.instance.step();
    const c = addEntity(scenario, options.spatial ? scenario.views[0].x : 25, options.spatial ? scenario.views[0].y : 25, options.is3D && options.spatial ? scenario.views[0].z : 0, 300);
    scenario.damage(c.entity, 275);
...
function createScenario(channelFactory, views, visible, manual) {
    var _a;
    const context = createContext();
    const instance = new Instance_1.Instance(context);
    const channel = channelFactory(instance);
    const writer = (_a = channel.createEntityWriter) === null || _a === void 0 ? void 0 : _a.call(channel, NType.Entity, context.getSchema(NType.Entity));
    ...
    const move = (entity, x, y, z) => {
        ...
        if (manual) {
            ...
            (_a = channel.updateEntity) === null || _a === void 0 ? void 0 : _a.call(channel, entity);
        }
    };
    const damage = (entity, hp) => {
        entity.hp = hp;
        if (manual) {
...
function allVisible(entity, view) {
    return true;
}
describe('object channel snapshot correctness', () => {
    it('keeps Channel clients synchronized', () => {
        runObjectScenario(createScenario(instance => new Channel_1.Channel(instance.localState, { name: 'channel-correctness' }), [{}, {}], allVisible, false));
    });
    it('keeps ManualChannel clients synchronized', () => {
        runObjectScenario(createScenario(instance => new ManualChannel_1.ManualChannel(instance.localState, { name: 'manual-correctness' }), [{}, {}], allVisible, true));
    });
    it('keeps SpatialChannel2D clients synchronized with the cell-coarse oracle', () => {
        runObjectScenario(createScenario(instance => new SpatialChannel2D_1.SpatialChannel2D(instance.localState, 10, { name: 'spatial-2d-correctness' }), [new AABB2D_1.AABB2D(5, 5, 10, 10), new AABB2D_1.AABB2D(105, 5, 10, 10)], visible2D, false));
    });
    it('keeps ManualSpatialChannel2D clients synchronized with the cell-coarse oracle', () => {
        runObjectScenario(createScenario(instance => new ManualSpatialChannel2D_1.ManualSpatialChannel2D(instance.localState, 10, { name: 'manual-spatial-2d-correctness' }), [new AABB2D_1.AABB2D(5, 5, 10, 10), new AABB2D_1.AABB2D(105, 5, 10, 10)], visible2D, true));
    });
    it('keeps SpatialChannel3D clients synchronized with the cell-coarse oracle', () => {
        runObjectScenario(createScenario(instance => new SpatialChannel3D_1.SpatialChannel3D(instance.localState, 10, { name: 'spatial-3d-correctness' }), [new AABB3D_1.AABB3D(5, 5, 5, 10, 10, 10), new AABB3D_1.AABB3D(105, 5, 5, 10, 10, 10)], visible3D, false));
    });
    it('keeps ManualSpatialChannel3D clients synchronized with the cell-coarse oracle', () => {
        runObjectScenario(createScenario(instance => new ManualSpatialChannel3D_1.ManualSpatialChannel3D(instance.localState, 10, { name: 'manual-spatial-3d-correctness' }), [new AABB3D_1.AABB3D(5, 5, 5, 10, 10, 10), new AABB3D_1.AABB3D(105, 5, 5, 10, 10, 10)], visible3D, true));
    });
    it('keeps Channel synchronized during deterministic churn', () => {
        runDeterministicChurnScenario(createScenario(instance => new Channel_1.Channel(instance.localState, { name: 'channel-churn-correctness' }), [{}, {}], allVisible, false), { spatial: false, is3D: false });
    });
    it('keeps ManualChannel synchronized during deterministic churn', () => {
        runDeterministicChurnScenario(createScenario(instance => new ManualChannel_1.ManualChannel(instance.localState, { name: 'manual-churn-correctness' }), [{}, {}], allVisible, true), { spatial: false, is3D: false });
    });
    it('keeps SpatialChannel2D synchronized during deterministic spatial churn', () => {
        runDeterministicChurnScenario(createScenario(instance => new SpatialChannel2D_1.SpatialChannel2D(instance.localState, 10, { name: 'spatial-2d-churn-correctness' }), [new AABB2D_1.AABB2D(5, 5, 10, 10), new AABB2D_1.AABB2D(105, 5, 10, 10)], visible2D, false), { spatial: true, is3D: false });
    });
    it('keeps ManualSpatialChannel2D synchronized during deterministic spatial churn', () => {
        runDeterministicChurnScenario(createScenario(instance => new ManualSpatialChannel2D_1.ManualSpatialChannel2D(instance.localState, 10, { name: 'manual-spatial-2d-churn-correctness' }), [new AABB2D_1.AABB2D(5, 5, 10, 10), new AABB2D_1.AABB2D(105, 5, 10, 10)], visible2D, true), { spatial: true, is3D: false });
    });
    it('keeps SpatialChannel3D synchronized during deterministic spatial churn', () => {
        runDeterministicChurnScenario(createScenario(instance => new SpatialChannel3D_1.SpatialChannel3D(instance.localState, 10, { name: 'spatial-3d-churn-correctness' }), [new AABB3D_1.AABB3D(5, 5, 5, 10, 10, 10), new AABB3D_1.AABB3D(105, 5, 5, 10, 10, 10)], visible3D, false), { spatial: true, is3D: true });
    });
    it('keeps ManualSpatialChannel3D synchronized during deterministic spatial churn', () => {
        runDeterministicChurnScenario(createScenario(instance => new ManualSpatialChannel3D_1.ManualSpatialChannel3D(instance.localState, 10, { name: 'manual-spatial-3d-churn-correctness' }), [new AABB3D_1.AABB3D(5, 5, 5, 10, 10, 10), new AABB3D_1.AABB3D(105, 5, 5, 10, 10, 10)], visible3D, true), { spatial: true, is3D: true });
    });
    it('keeps Channel synchronized when queued snapshots are drained later', () => {
        runQueuedSnapshotScenario(createScenario(instance => new Channel_1.Channel(instance.localState, { name: 'channel-queued-correctness' }), [{}, {}], allVisible, false), { spatial: false, is3D: false });
    });
    it('keeps ManualChannel synchronized when queued snapshots are drained later', () => {
        runQueuedSnapshotScenario(createScenario(instance => new ManualChannel_1.ManualChannel(instance.localState, { name: 'manual-queued-correctness' }), [{}, {}], allVisible, true), { spatial: false, is3D: false });
    });
    it('keeps SpatialChannel2D synchronized when queued snapshots are drained later', () => {
        runQueuedSnapshotScenario(createScenario(instance => new SpatialChannel2D_1.SpatialChannel2D(instance.localState, 10, { name: 'spatial-2d-queued-correctness' }), [new AABB2D_1.AABB2D(5, 5, 10, 10), new AABB2D_1.AABB2D(105, 5, 10, 10)], visible2D, false), { spatial: true, is3D: false });
    });
    it('keeps ManualSpatialChannel2D synchronized when queued snapshots are drained later', () => {
        runQueuedSnapshotScenario(createScenario(instance => new ManualSpatialChannel2D_1.ManualSpatialChannel2D(instance.localState, 10, { name: 'manual-spatial-2d-queued-correctness' }), [new AABB2D_1.AABB2D(5, 5, 10, 10), new AABB2D_1.AABB2D(105, 5, 10, 10)], visible2D, true), { spatial: true, is3D: false });
    });
    it('keeps SpatialChannel3D synchronized when queued snapshots are drained later', () => {
        runQueuedSnapshotScenario(createScenario(instance => new SpatialChannel3D_1.SpatialChannel3D(instance.localState, 10, { name: 'spatial-3d-queued-correctness' }), [new AABB3D_1.AABB3D(5, 5, 5, 10, 10, 10), new AABB3D_1.AABB3D(105, 5, 5, 10, 10, 10)], visible3D, false), { spatial: true, is3D: true });
    });
    it('keeps ManualSpatialChannel3D synchronized when queued snapshots are drained later', () => {
        runQueuedSnapshotScenario(createScenario(instance => new ManualSpatialChannel3D_1.ManualSpatialChannel3D(instance.localState, 10, { name: 'manual-spatial-3d-queued-correctness' }), [new AABB3D_1.AABB3D(5, 5, 5, 10, 10, 10), new AABB3D_1.AABB3D(105, 5, 5, 10, 10, 10)], visible3D, true), { spatial: true, is3D: true });
    });
});
