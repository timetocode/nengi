...
function trackRaw(history, player, timeMs = 0) {
...
function trackPublic(history, player, timeMs = 0) {
    history.trackSpatial(player, {
        nid: 'nid',
        x: 'publicX',
        y: 'publicY',
...
function closestEnemyHit(history, timeMs, shooter, targetX, targetY) {
...
describe('Historian2D gameplay policy sketches', () => {
    it('supports traditional attacker-time hitscan rewind without owning the damage policy', () => {
        ...
        const nowTimeMs = 150;
        expect((_a = closestEnemyHit(history, shotTimeMs, shooter, 140, 0)) === null || _a === void 0 ? void 0 : _a.sample.nid).toBe(target.nid);
    ...
    it('supports peeker-compromise trades as userland policy over two valid historical shots', () => {
        ...
        expect(peekerHit === null || peekerHit === void 0 ? void 0 : peekerHit.sample.nid).toBe(defender.nid);
        expect(defenderHit === null || defenderHit === void 0 ? void 0 : defenderHit.sample.nid).toBe(peeker.nid);
        expect(allowTrade).toBe(true);
    });
    it('can track public/smoothed bodies instead of raw bodies for observer-facing hit rules', () => {
        var _a;
        const rawHistory = new Historian2D_1.Historian2D({ retentionMs: 1000 });
        const publicHistory = new Historian2D_1.Historian2D({ retentionMs: 1000 });
        const shooter = { nid: 1, rawX: 0, rawY: 0, publicX: 0, publicY: 0, radius: 12, team: 1 };
        ...
        expect((_a = closestEnemyHit(rawHistory, 100, shooter, 140, 0)) === null || _a === void 0 ? void 0 : _a.sample.nid).toBe(target.nid);
    ...
    it('can return historical samples for ids that no longer exist in current game state', () => {
        ...
        expect(historicalHit === null || historicalHit === void 0 ? void 0 : historicalHit.sample.nid).toBe(target.nid);
    ...
    it('supports delayed deletion as userland death lifecycle rather than historian policy', () => {
        const history = new Historian2D_1.Historian2D({ retentionMs: 1000 });
        const bodiesByNid = new Map();
        const shooter = { nid: 1, rawX: 0, rawY: 0, publicX: 0, publicY: 0, radius: 12, team: 1, hp: 100, state: 'alive' };
        const target = { nid: 2, rawX: 100, rawY: 0, publicX: 100, publicY: 0, radius: 12, team: 2, hp: 5, state: 'alive' };
        bodiesByNid.set(shooter.nid, shooter);
        bodiesByNid.set(target.nid, target);
        trackRaw(history, shooter);
        trackRaw(history, target);
        ...
        const historicalHit = closestEnemyHit(history, 100, shooter, 140, 0);
        const currentTarget = historicalHit ? bodiesByNid.get(historicalHit.sample.nid) : undefined;
        const canApplyNormalDamage = (currentTarget === null || currentTarget === void 0 ? void 0 : currentTarget.state) === 'alive';
        const canApplyTradeCredit = (currentTarget === null || currentTarget === void 0 ? void 0 : currentTarget.state) === 'dead' && history.getValue(target.nid, 'lifeState', 100) !== 'dead';
        expect(historicalHit === null || historicalHit === void 0 ? void 0 : historicalHit.sample.nid).toBe(target.nid);
