# Prompt: Minimal Local Game Prototype

Use this prompt to ask a fresh AI assistant to build a small game in this R&D
workspace and report on the nengi developer experience.

```text
You are working in the local nengi R&D workspace.

Before creating files, verify that this shell can run the local toolchain:
...
When using official adapter packages, use the `shared/nengi.ts` shim from
local-prototype.md, which re-exports from package `nengi`. Do not mix
`../../../nengi/src` imports with adapter packages.
...
- Make a very small 2D multiplayer game.
...
- Keep the gameplay intentionally small; this is an engine/API proof of concept,
  not a full game.
...
Keep the code straightforward. Prefer clarity over adding advanced features.
Do not add prediction, inventories, ECS, historian lag compensation, bots,
benchmarks, or manual mutation channels unless they naturally become necessary
for the tiny game.
```
